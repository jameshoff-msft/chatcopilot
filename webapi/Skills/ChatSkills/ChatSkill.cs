...
public class ChatSkill
{
    ...
    {
        string index = "";
        if (context.Variables.ContainsKey("index"))
        {
        ...
        else
        {
            ...
            chatMessage = await this.GetChatResponseAsync(chatId, userId, chatContext, cancellationToken, index);
        }
    ...
    private async Task<ChatMessage> GetChatResponseAsync(string chatId, string userId, SKContext chatContext, CancellationToken cancellationToken, string index)
    {
        // Get the audience
        ...
        string[] tasks = await Task.WhenAll<string>(
            ...
            this._documentMemorySkill.QueryDocumentsAsync(userIntent, chatId, documentContextTokenLimit, this._kernel.Memory, index, cancellationToken)
        );
